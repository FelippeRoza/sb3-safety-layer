...
def main(args):
    ...
    if args.method == 'unsafe':
        rl_agent = SAC('MlpPolicy', env, verbose=1, tensorboard_log=os.path.join('data', 'experiments', args.env_name, 'tensorboard'))
    else:
        sl = CDM(env, buffer_size=args.sl_buffer_size)
        sl.load(os.path.join('data', 'pretrained_cdm', args.env_name))
        rl_agent = PPO(SafeActorCriticPolicy, env, verbose=1, tensorboard_log=os.path.join('data', 'experiments', 'tensorboard'),
